...
use tamper_audit::{AuditEntry, AuditLog, AuditChain, AuditQuery};

fn bench_audit_entry_creation(c: &mut Criterion) {
    let genesis_hash = "0".repeat(64);
    c.bench_function("audit_entry_creation", |b| {
        b.iter(|| {
            AuditEntry::new("alice", "create", "user/1", serde_json::json!({"name": "Alice"}), &genesis_hash)
        });
    });
}

fn bench_audit_entry_compute_hash(c: &mut Criterion) {
    let entry = AuditEntry::new("alice", "create", "user/1", serde_json::json!({}), &"0".repeat(64));
...
fn bench_audit_log_append_chained(c: &mut Criterion) {
    c.bench_function("audit_log_append_chained", |b| {
        b.iter_with_setup(
            ...
            |mut log: AuditLog| {
                log.append("bob", "update", "user/1", serde_json::json!({"name": "Bob"})).unwrap();
            },
        );
    });
}
...
fn bench_audit_query_by_actor(c: &mut Criterion) {
    ...
    for i in 0..100 {
        ...
        log.append(actor, "create", "resource", serde_json::json!({})).unwrap();
    }
    c.bench_function("audit_query_by_actor", |b| {
        b.iter(|| log.query_by_actor("alice"));
    });
}
...
fn bench_audit_query_by_action(c: &mut Criterion) {
    let mut log = AuditLog::new();
    for i in 0..100 {
        let action = if i % 3 == 0 { "create" } else if i % 3 == 1 { "update" } else { "delete" };
        log.append("user", action, "resource", serde_json::json!({})).unwrap();
    }
    c.bench_function("audit_query_by_action", |b| {
        b.iter(|| log.query_by_action("create"));
    });
}
...
fn bench_audit_query_by_resource(c: &mut Criterion) {
    ...
    for i in 0..100 {
        ...
        log.append("user", "update", resource, serde_json::json!({})).unwrap();
    }
    c.bench_function("audit_query_by_resource", |b| {
        b.iter(|| log.query_by_resource("resource/5"));
    });
}
